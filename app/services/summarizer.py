...
תוכן השדות (המבנה נאכף אוטומטית — אתה אחראי לאיכות התוכן):
  ...
  • quiz — an exam with 8-10 questions (Bloom's levels 1-6) with 4 options, a correct_answer identical to one of the options, and an explanation.
...
אסור בהחלט — שאלות מהסוגים הבאים פסולות:
  ...
  ❌ שאלות "נכון/לא נחות" או "מי מהבאים אינו..." — שאלות פשוטות מדי שאינן דורשות ניתוח
  ❌ שאלות המבוססות על רשימות מפורשות (למש एग्जांपल: "מהם שלושת השלבים ב-X?") — אלו דורשות רק זיהוי ולא הבנה
  ❌ שאלות שהתשובה שלהן נובעת מהסרת אפשרות אחת בלבד באמצעות לוגיקה פשוטה (elimination)
